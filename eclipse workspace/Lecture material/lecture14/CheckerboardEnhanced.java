 ...
 * @author sehall
 */
public class CheckerboardEnhanced extends JPanel implements ActionListener{
    public final int PANEL_WIDTH = 700;
    public final int PANEL_HEIGHT = 700;
    private DrawingPanel drawPanel;  //innerclass
    private JRadioButton draughtsButton, chessButton, discoButton;
    private Color alternateColor;
    private Timer timer;
    
    public CheckerboardEnhanced() {
        ...
        discoButton = new JRadioButton("disco seizure board");
    ...
    {   private Random gen;
        ...
        public void paintComponent(Graphics g){
            ...
            {   //loop x coordinate, used to shift 
                for(int j=0;j<8;j++)
                {   if(!discoButton.isSelected())
                    {   if((i+j) %2 == 0)
                            g.setColor(alternateColor);
                        else g.setColor(Color.BLACK);
                    }
                     else 
                        g.setColor(new Color(gen.nextFloat(),gen.nextFloat(),gen.nextFloat())); 
                    //formula to shift x,y position of box
    ...
    public static void main(String[] args) {
        CheckerboardEnhanced myPanel = new CheckerboardEnhanced();
