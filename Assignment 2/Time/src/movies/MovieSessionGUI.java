	...
	private void createseatingButtons()
	{
		...
		for (int i = 0; i < MovieSession.NUM_ROWS; i++)
		{
			for (int i2 = 0; i2 < MovieSession.NUM_COLS; i2++)
			{
				...
				// If the seating button is pressed then perform the following actions:
				public void actionPerformed(ActionEvent e) {
		...
		list.addListSelectionListener(new ListSelectionListener() {
			public void valueChanged(ListSelectionEvent e) {
				...
				if (e.getValueIsAdjusting() == false)
				{
					...
					else if (currentReservation.isEmpty() == false && list.getSelectedIndex() != e.getFirstIndex()) 
					{
						...
						{
							list.setSelectedIndex(e.getFirstIndex()); // Else stay on the current selection.
						}
					}
